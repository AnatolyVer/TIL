...
const useSounds = () => {
    const playCorrect = React.useCallback(() => {
        const audio = new Audio("/sounds/correct_answer.wav");
        audio.volume = 1;
        audio.play().catch(() => {});
    }, []);

    const playWrong = React.useCallback(() => {
        const audio = new Audio("/sounds/wrong_answer.wav");
        audio.volume = 1;
        audio.play().catch(() => {});
    }, []);
...
const Page = () => {
    ...
    const [shuffledQuestion, setShuffledQuestion] = React.useState<ShuffledQuestion | null>(null);
    ...
    // ──────────────────────────────────────────────
    // Функция перемешивания вопроса
    // ──────────────────────────────────────────────
    const shuffleQuestion = (recipeIndex: number, questionIndex: number) => {
        const q = recipes[recipeIndex].questions[questionIndex];
    ...
    const handleAnswer = (index: number) => {
        if (selectedAnswer !== null || !shuffledQuestion) return;

        setSelectedAnswer(index);

        const isCorrect = shuffledQuestion.answers[index].isCorrect;
        ...
        setTimeout(() => {
            ...
            const qLen = recipes[selectedRecipe].questions.length;

            if (currentQuestion < qLen - 1) {
                const next = currentQuestion + 1;
                setCurrentQuestion(next);
                shuffleQuestion(selectedRecipe, next);
                setSelectedAnswer(null);
            } else {
                setGameState("result");
            }
    ...
    // Добавление блюда при идеальном прохождении
    React.useEffect(() => {
        if (gameState === "result" && isPerfect && !prevPerfectRef.current && currentRecipe) {
    ...
    return (
        <>
            <div className={`relative ${isLandscape ? "block" : "hidden"} flex justify-center items-end w-full min-h-[calc(100vh-72px)]`}>
                ...
                <Image src="/floor.png" alt="floor" width={1600} height={1600} priority className="absolute bottom-0 w-full h-1/2 z-2"/>
                <Image src="/wall.png" alt="wall" width={1600} height={1600} priority className="absolute -top-5 w-full h-9/10 z-1"/>
                ...
                {isModalOpen && (
                    <div className="absolute inset-0 bg-black/70 flex justify-center items-center z-50">
                        <div className="w-11/12 max-w-lg p-8 bg-white rounded-2xl shadow-2xl flex flex-col gap-6">
                            ...
                            {/* Викторина */}
                            {gameState === "quiz" && shuffledQuestion && (
                                <div className="text-center">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-8">
                                        {shuffledQuestion.answers.map((option, index) => (
                                            <button
                                                key={index}
                                                onClick={() => handleAnswer(index)}
                                                disabled={selectedAnswer !== null}
                                                className={`
                                                    py-4 px-6 rounded-xl cursor-pointer font-medium transform transition-all
                                                    ${selectedAnswer === null
                                    ...
                                    <div className="text-4xl mt-6 text-pink-600">
                                        {selectedAnswer !== null &&
                                            (shuffledQuestion.answers[selectedAnswer].isCorrect ? "Отлично!" : "Не совсем!")}
                                    </div>
                                </div>
                            )}
