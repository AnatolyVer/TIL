...
export default function Player({
    ...
    setIsPlayerError: (value: boolean) => void;
}) {
    ...
    useEffect(() => {
        ...
        window.onSpotifyWebPlaybackSDKReady = () => {
            ...
            spotifyPlayer.addListener("player_state_changed", async (state) => {
                ...
                const repeatMode = repeatModes[state.repeat_mode] as
                    | "off"
                    | "track"
                    | "context";
                    ...
                    <div className="flex items-center absolute lg:static left-0 ml-5 top-4 lg:justify-start lg:ml-0 w-3/5 lg:w-2/5 overflow-hidden">
                        ...
                        <div className="ml-3 flex flex-col overflow-hidden w-full">
                            <div ref={containerRef} className="relative overflow-hidden w-full">
                                <div
                                    ref={titleRef}
                                    className={`text-sm sm:text-base font-semibold whitespace-nowrap flex ${
                                        isOverflowing ? "animate-marquee-loop" : ""
                                    }`}
                                >
                                    <span className="mr-7">{PlayerState.currentTrack.name}</span>
                                    {isOverflowing && (
                                        <span className="mr-7">{PlayerState.currentTrack.name}</span>
                                    )}
                                </div>
